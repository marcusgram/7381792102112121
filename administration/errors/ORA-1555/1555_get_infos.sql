...
------------------------------------------------------------------------------------------------------------------------------------------------------
--      Find out what caused an ORA-1555 error
--
--      Script      ora1555.sql
--      Run as      DBA
--
--      Purpose     If you want to find out what caused an ORA-1555 error, it's
--                  best not to wait too long to find out. It is kind of handy
--                  then to have a script that catches all necessary information
--                  for that. This script aims to do just that.
--
--      Input       SQL_ID, SCN (see error message in alert log), and child_number.
--
--      Author      M. Krijgsman
--
--      Opm.    Geen
...
--      ------- ----------- -------------- ----------------------------------------------------------------------------------------------
--      1.0     13 jan 2012 M. Krijgsman   Initieel
--      1.1     17 jan 2012 M. Krijgsman   Gepoogd in te vullen SCN te verduidelijken
--      1.2     20 jan 2012 M. Krijgsman   "Wie heeft de query uitgevoerd?" sectie toegevoegd
--	1.3	25 apr 2012 M. Krijgsman   undo_retention parameter meegenomen in overzicht.
--	1.4     02 aug 2012 M. Krijgsman   v$sql_bind_capture gegevens en bind mismatch info toegevoegd.
--      1.5     19 okt 2012 M. Krijgsman   Bind variable commando's voor SQL*Plus van sample binds.
--      1.6     22 nov 2012 M. Krijgsman   English version, hash value info, sql plan baseline info included.
--      1.7     27 dec 2012 M. Krijgsman   SQL profile info, query response time history
--      1.8     15 feb 2013 M. Krijgsman   Brought this script on par with sql_sql_id.sql version 1.8
------------------------------------------------------------------------------------------------------------------------------------------------------
...
spool ora1555_&l_dbname._&datetime..txt

prompt =============================================
prompt =                                           =
prompt =            ORA-01555.sql                  =
prompt =  This script gathers a lot of information =
prompt =        about ORA-01555 errors.            =
prompt =                                           =
prompt =  Run this script on the instance where    =
prompt =          the error occurred.              =
prompt =                                           =
prompt =============================================
prompt
...
prompt Undo_retention parameter
...
select 'SESSION' "WHERE", name, value
from v$parameter
where name='undo_retention'
union
select 'MEMORY' "WHERE", name, value
from V$SYSTEM_PARAMETER
where name='undo_retention'
union
select 'SPFILE' "WHERE", name, value from v$spparameter
where name='undo_retention'
;
...
-----------------------------------------------------------------------
-- Input of information found in the ORA-1555 message in the alertlog.
-----------------------------------------------------------------------

accept sql_id    default '' -
  prompt 'What SQL ID is mentioned in the ORA-01555 message?: '

prompt
prompt

prompt Please provide the SCN:
prompt o Note the SCN in the error message. We need this without 0x at the beginning
prompt   and without the dot in between.
prompt o Example: original SCN: 0x0000.2c22631f --> your input: 00002c22631f
prompt o If mentioned, chose the Recent SCN.

prompt

accept scnnr    default '' -
  prompt 'What SCN is mentioned in the ORA-01555 message?: '

prompt
prompt

prompt Time the transaction started
prompt (This helps to find the real start time, when Query Duration=0 in the message)

select scn_to_timestamp(to_number('&scnnr', 'XXXXXXXXXXXX')) 
from dual;


prompt Queries that used the most undo today.
prompt

col begin_time for a30

select to_char(begin_time,'DD-MON-YY HH24:MI:SS') begin_time ,inst_id, MAXQUERYID, MAXQUERYLEN 
from gv$undostat 
where trunc(begin_time) = trunc(sysdate)
order by begin_time, inst_id asc;
